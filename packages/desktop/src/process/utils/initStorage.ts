...
const STORAGE_PATH = {
  config: 'aura-config.txt',
  chatMessage: 'aura-chat-message.txt',
  chat: 'aura-chat.txt',
  env: '.aionui-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[AURA] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[AURA] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[AURA] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('aura.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'aura-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'aura-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'aura-chat-history', conversation_id + '.txt'));
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[AURA] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `crates/aura-app/assets/builtin-assistants/` and
 * `crates/aura-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[AURA:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[AURA] Failed to ensure assistant dirs:', error);
  }
