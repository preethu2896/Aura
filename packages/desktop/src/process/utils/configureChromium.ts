...
function findAvailablePort(preferredPort: number): number {
  ...
  console.log(
    `[CDP] Port ${preferredPort} is occupied by another AURA instance, scanning range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}`
  );
  ...
  console.warn(
    `[CDP] All ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END} are used by active AURA instances, trying ${preferredPort}`
  );
  return preferredPort;
}
