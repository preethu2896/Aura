...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'aura');
};
...
export const getDataPath = (): string => {
  ...
  const dataPath = path.join(rootPath, 'aura');
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[AURA] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[AURA] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[AURA] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[AURA] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
