...
function isBackendStartupFailureEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.['aura.failure'] === 'backend_startup';
}

function isUserFeedbackEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.type === 'user-feedback' || event.tags?.['aura.installation_integrity.user_report'] === 'true';
}
...
function getInstallPathKind(resourcesPath: unknown): string | undefined {
  ...
  if (normalized.includes('\\appdata\\local\\programs\\aura\\resources')) {
    return 'user_local_programs';
  }
  if (
    normalized.includes('\\program files\\aura\\resources') ||
    normalized.includes('\\program files (x86)\\aura\\resources')
  ) {
...
export async function captureBackendStartupFailure(error: unknown): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.setTag('aura.failure', 'backend_startup');
    scope.setTag('aura.backend_startup.reason', failureInfo.reason);
    if (failureInfo.runtime) {
      scope.setTag('aura.backend_startup.runtime', failureInfo.runtime);
    }
    if (failureInfo.packageArch) {
      scope.setTag('aura.backend_startup.package_arch', failureInfo.packageArch);
    }
    if (failureInfo.deviceArch) {
      scope.setTag('aura.backend_startup.device_arch', failureInfo.deviceArch);
    }
    if (failureInfo.expectedDownloadArch) {
      scope.setTag('aura.backend_startup.expected_download_arch', failureInfo.expectedDownloadArch);
    }
    if (typeof failureInfo.isRosettaTranslated === 'boolean') {
      scope.setTag('aura.backend_startup.rosetta_translated', getBooleanTagValue(failureInfo.isRosettaTranslated));
    }
    if (typeof details?.stage === 'string') {
      scope.setTag('aura.backend_startup.stage', details.stage);
    }
    if (failureInfo.backendBoundaryCode) {
      scope.setTag('aura.backend_startup.boundary_code', failureInfo.backendBoundaryCode);
    }
    if (failureInfo.backendBoundaryStage) {
      scope.setTag('aura.backend_startup.boundary_stage', failureInfo.backendBoundaryStage);
    }
    if (failureInfo.localDataIssueKind) {
      scope.setTag('aura.backend_startup.local_data_issue_kind', failureInfo.localDataIssueKind);
    }
    if (failureInfo.incompleteInstallationKind) {
      scope.setTag('aura.backend_startup.incomplete_installation_kind', failureInfo.incompleteInstallationKind);
    }
    for (const [tag, value] of [
      ['aura.backend_startup.missing_bundled_dir', getBooleanTagValue(failureInfo.missingBundledAioncoreDir)],
      ['aura.backend_startup.missing_runtime_dir', getBooleanTagValue(failureInfo.missingRuntimeDir)],
      ['aura.backend_startup.missing_binary', getBooleanTagValue(failureInfo.missingBackendBinary)],
      ['aura.backend_startup.missing_hub_dir', getBooleanTagValue(failureInfo.missingHubDir)],
      ['aura.backend_startup.missing_pet_states_dir', getBooleanTagValue(failureInfo.missingPetStatesDir)],
      ['aura.backend_startup.missing_pwa_dir', getBooleanTagValue(failureInfo.missingPwaDir)],
      ['aura.backend_startup.install_path_kind', getInstallPathKind(details?.resourcesPath)],
      ['aura.backend_startup.last_update_status', getString(autoUpdateDiagnostics?.lastEvent?.status)],
      [
        'aura.backend_startup.health_polling_delayed',
        getBooleanTagValue(
          typeof details?.healthCheckPollingDelayed === 'boolean' ? details.healthCheckPollingDelayed : undefined
        ),
      ],
      ['aura.backend_startup.health_attempts_bucket', getHealthAttemptBucket(details?.healthCheckAttempts)],
      [
        'aura.backend_startup.health_attempt_deficit_bucket',
        getHealthAttemptBucket(details?.healthCheckAttemptDeficit),
      ],
      ['aura.backend_startup.health_timeout_overrun_bucket', getDurationBucket(details?.healthCheckTimeoutOverrunMs)],
      ['aura.backend_startup.health_max_attempt_gap_bucket', getDurationBucket(details?.healthCheckMaxAttemptGapMs)],
      [
        'aura.backend_startup.seconds_since_quit_and_install',
