...
if (!gotTheLock) {
  console.warn('[AURA] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[AURA] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[AURA] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[AURA] runBackendMigrations completed');
    } catch (error) {
      console.error('[AURA] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[AURA] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  ...
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[AURA] Ignoring AIONUI_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[AURA] Ignoring unknown AIONUI_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[AURA] Creating main window...');
  ...
  console.log(`[AURA] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[AURA] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[AURA] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[AURA] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[AURA] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[AURA] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[AURA] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[AURA] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[AURA] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[AURA] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[AURA] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[AURA] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[AURA] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[AURA] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[AURA] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[AURA] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[AURA] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[AURA:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[AURA] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[AURA] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`AURA --webui` or packaged `aura-web` mode that
      ...
      const handle = await startWebHost({
        ...
        // Expose the same AURA_{CACHE,WORK,LOG}_DIR env the desktop IPC path
...
// Register aura:// as the default protocol client
...
// macOS: handle aura:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[AURA] App initialization failed:', error);
    app.quit();
  });
...
app.on('will-quit', () => {
  console.log('[AURA] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[AURA] quit (exitCode=${exitCode})`);
});
