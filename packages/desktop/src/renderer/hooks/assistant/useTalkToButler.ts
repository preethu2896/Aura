...
/** Backend manifest id of the built-in AURA Butler assistant. */
 ...
 * select the AURA Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the AURA Butler for you' })
            );
          }
        }
      } catch (error) {
