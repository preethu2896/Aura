...
const AgentRepairPanel: React.FC<AgentRepairPanelProps> = ({ agent, onSaved }) => {
  ...
  const envBlock = (
    <div>
      ...
      {/* What configuring env vars can fix — grounded in how AURA injects them
  ...
  return (
    <div className='mt-10px flex flex-col gap-12px rounded-10px bg-aou-1 px-12px py-12px'>
      ...
      {!isInternalAURACli && showPath ? pathBlock : null}
      {!isInternalAURACli ? envBlock : null}
