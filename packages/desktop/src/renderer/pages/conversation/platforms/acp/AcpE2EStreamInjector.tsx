...
const ENABLED_CONVERSATION_KEY = 'aura:e2e-message-stream-conversation-id';
...
declare global {
  interface Window {
    __AURA_E2E_MESSAGE_STREAM__?: StreamRegistry;
  }
}
...
const AcpE2EStreamInjector: React.FC<{ conversationId: string }> = ({ conversationId }) => {
  ...
  useEffect(() => {
    ...
    const registry = (window.__AURA_E2E_MESSAGE_STREAM__ ??= { controllers: {} });
    ...
    return () => {
      if (window.__AURA_E2E_MESSAGE_STREAM__) {
        delete window.__AURA_E2E_MESSAGE_STREAM__.controllers[conversationId];
      }
    };
