 ...
 */
export interface AURACollapseProps {
 ...
 */
export interface AURACollapseItemProps {
 ...
 */
const AURACollapseItem: React.FC<AURACollapseItemProps> = ({ children }) => <>{children}</>;
AURACollapseItem.displayName = 'AURACollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <AURACollapse defaultActiveKey={['1']}>
 *   <AURACollapse.Item name="1" header="面板1">
 *     内容1
 *   </AURACollapse.Item>
 *   <AURACollapse.Item name="2" header="面板2">
 *     内容2
 *   </AURACollapse.Item>
 * </AURACollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <AURACollapse accordion defaultActiveKey="1">
 *   <AURACollapse.Item name="1" header="面板1">内容1</AURACollapse.Item>
 *   <AURACollapse.Item name="2" header="面板2">内容2</AURACollapse.Item>
 * </AURACollapse>
 *
 * // 自定义图标 / Custom icon
 * <AURACollapse
 ...
 * ```
 */
const AURACollapseComponent: React.FC<AURACollapseProps> & { Item: typeof AURACollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<AURACollapseItemProps> => {
      return React.isValidElement(child) && child.type === AURACollapseItem;
    });
  }, [children]);
...
AURACollapseComponent.Item = AURACollapseItem;

export default AURACollapseComponent;
