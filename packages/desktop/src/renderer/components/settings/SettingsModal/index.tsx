...
import AURAModal from '@/renderer/components/base/AURAModal';
...
export const SubModal: React.FC<SubModalProps> = ({ visible, onCancel, title, children }) => {
  return (
    <AURAModal
      ...
      title={title}
    >
      <AURAScrollArea className='h-full px-20px pb-16px text-14px text-t-primary'>{children}</AURAScrollArea>
    </AURAModal>
  );
};
...
const SettingsModal: React.FC<SettingsModalProps> = ({ visible, onCancel, defaultTab = 'model' }) => {
  ...
  const desktopMenu = (
    <AURAScrollArea className='flex-shrink-0 b-color-border-2 scrollbar-hide' style={{ width: `${SIDEBAR_WIDTH}px` }}>
