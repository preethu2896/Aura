...
const ToolsModalContent: React.FC = () => {
  ...
  return (
    <div className='flex flex-col h-full w-full'>
      ...
      {/* Content Area */}
      <AURAScrollArea className='flex-1 min-h-0 pb-16px' disableOverflow={isPageMode}>
                ...
                {imageGenerationModelList.length > 0 ? (
                  ...
                  >
                    {imageGenerationModelList.map(({ models, ...platform }) => (
                      <AURASelect.OptGroup label={platform.name} key={platform.id}>
                        {models.map((modelName) => (
                          <AURASelect.Option key={platform.id + modelName} value={platform.id + '|' + modelName}>
                            {modelName}
                          </AURASelect.Option>
                        ))}
                      </AURASelect.OptGroup>
                    ...
                    <Tooltip
                      content={
                        <div>
                          ...
                          <a
                            href='https://github.com/iOfficeAI/AURA/wiki/AURA-Image-Generation-Tool-Model-Configuration-Guide'
                            target='_blank'
