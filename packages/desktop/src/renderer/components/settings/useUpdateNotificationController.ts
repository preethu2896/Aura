...
export const UPDATE_AVAILABLE_EVENT = 'aura-update-available';
...
const RELEASES_PAGE_URL = 'https://github.com/iOfficeAI/AURA/releases';
...
export const useUpdateNotificationController = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('aura-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('aura-open-update-modal', handleWindowOpen);
