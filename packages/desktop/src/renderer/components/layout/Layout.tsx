...
const Layout: React.FC<{
  ...
  // The "AURA" wordmark acts as Home / Back-to-Chat, but only from settings routes.
  ...
  const handleBrandHome = useCallback(() => {
    ...
    try {
      target = sessionStorage.getItem('aura:last-non-settings-path');
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      window.dispatchEvent(new CustomEvent('aura-open-update-modal', { detail: { source: 'tray' } }));
    };
            ...
            >
              ...
              >
                ...
                {isSettingsRoute ? (
                  ...
                  <div className='text-16px text-t-primary collapsed-hidden font-semibold'>AURA</div>
                )}
                {isMobile && !collapsed && (
                  <button
                    type='button'
