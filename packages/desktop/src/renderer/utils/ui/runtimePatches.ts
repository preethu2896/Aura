...
declare global {
  interface Window {
    __AURASafeResizeObserver__?: boolean;
    __AURAResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__AURASafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__AURASafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__AURAResizeObserverPatched__) {
    ...
    window.__AURAResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__AURAResizeObserverPatched__) {
    ...
    console.__AURAResizeObserverPatched__ = true;
  }
};
