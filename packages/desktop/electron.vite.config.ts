...
// Read the real AURA version from the repo-root package.json.
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another AURA clone is running),
      ...
      define: {
        ...
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real AURA version (root package.json) so renderer code
