 ...
 * /login and /logout to aioncore. All auth goes to backend's aura-auth crate;
 * /login and /logout are aura-auth's top-level paths, the rest live under
...
export async function startStaticServer(opts: StaticServerOptions): Promise<StaticServerHandle> {
  ...
  const http_server: Server = http.createServer(async (req, res) => {
    try {
      ...
      // /login and /logout are aura-auth's top-level auth endpoints: proxy them too
