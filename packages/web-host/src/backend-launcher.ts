...
export type BackendLaunchOptions = {
  ...
  logDir?: string;
  /**
   * System dirs exposed to the backend via AURA_{CACHE,WORK,LOG}_DIR env.
...
/**
 * Backend reads AURA_{CACHE,WORK,LOG}_DIR env vars to report system dirs
 * (see AionCore/crates/aura-system/src/sysinfo.rs). Inject them so the
