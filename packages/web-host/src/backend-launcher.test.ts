...
describe('buildSpawnEnv', () => {
  it('merges process.env with AURA_* dir vars', () => {
...
describe('BackendLifecycleManager.start (health timeout)', () => {
  it('captures backend boundary code and stage from early-exit stderr', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.open databasePath=/db/path/aura-backend.db: failed to initialize application data\n'
      )
    );
  ...
  it('captures backend boundary code when stderr drains after exit but before close', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.migration databasePath=/db/path/aura-backend.db: failed to initialize application data\n'
      )
    );
    child.emit('close', 1, null);
