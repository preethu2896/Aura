...
export type EnsureAdminPasswordOptions = {
   ...
   * by launch context — packaged tarball = `aura-web resetpass`, in-repo dev
...
export async function ensureAdminPassword(
  ...
  const resetCmd = opts.resetCommand ?? 'aura-web resetpass';
  ...
  try {
    ...
    deps.warn(`[aura-web] could not verify admin credentials: ${err instanceof Error ? err.message : String(err)}`);
    return;
  }
  ...
  if (!needsSetup) {
    ...
    deps.log(`[aura-web] Log in with username "${username}". Forgot the password? Run \`${resetCmd}\`.`);
    return;
  }
  ...
  try {
    ...
    if (!resetRes.ok) {
      deps.warn(`[aura-web] /api/webui/reset-password returned ${resetRes.status} — run \`${resetCmd}\``);
      return;
    }
    ...
    if (!newPassword) {
      deps.warn(`[aura-web] /api/webui/reset-password returned no new_password — run \`${resetCmd}\``);
      return;
    }
    const username = await fetchAdminUsername(deps, opts.backendPort);
