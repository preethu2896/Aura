...
// tarball layout:
//   aura-web/
//   ├── aura-web              ← bun-compiled standalone binary (process.execPath)
...
const isPackaged = (() => {
  ...
  return exeName === 'aura-web' || exeName === 'aura-web.exe';
})();

const BACKEND_BINARY = process.platform === 'win32' ? 'aioncore.exe' : 'aioncore';
const DEFAULT_PORT = 25808;
const RESET_COMMAND = isPackaged ? 'aura-web resetpass' : 'bun run resetpass';
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[aura-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[aura-web] version    : ${version}`);
  console.log(`[aura-web] data dir   : ${dataDir}`);
  console.log(`[aura-web] log dir    : ${logDir}`);
  console.log(`[aura-web] static dir : ${staticDir}`);
  console.log(`[aura-web] backend bin: ${backendBin}`);
  console.log(`[aura-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.log('');
    console.log('AURA WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[aura-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[aura-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
    ...
    console.log('');
    console.log('AURA WebUI is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[aura-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[aura-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `aura-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[aura-web] backend binary not found: ${backendBin}`);
  ...
  console.log(`[aura-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[aura-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[aura-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[aura-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[aura-web] username: ${username}`);
    console.log(`[aura-web] new password: ${newPassword}`);
    console.log('[aura-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: aura-web <command> [options]
...
`);
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: aura-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[aura-web] fatal:', err.message);
