...
AURA development uses two repositories:
...
- **AURA** (`https://github.com/iOfficeAI/AURA.git`) starts the Electron desktop app and launches the backend binary automatically.
...
The desktop development server resolves the backend from the `PATH` inherited by `bun run start`. Install AionCore first, verify the binary is discoverable in the same terminal, then start AURA.
...
cd AionCore
cargo clean
cargo install --path crates/aura-app --locked

# Make Cargo-installed binaries visible to this shell if needed.
export PATH="$HOME/.cargo/bin:$PATH"

# Verify that AURA will be able to find the backend.
which aioncore
aioncore --help
```
...
Run these commands from the `AURA` repository in a terminal where `aioncore` is discoverable.
...
During startup, AURA launches `aioncore` automatically and passes the backend port to the renderer. You do not need to start AionCore in a separate terminal.

## Updating the Local Backend

When you pull or change AionCore, reinstall the backend binary and restart AURA:

```bash
cd ../AionCore
cargo install --path crates/aura-app --locked --force
...
AURA cannot find the backend from the `PATH` inherited by `bun run start`.

Check from the same terminal where you start AURA:
...
If the command fails, add Cargo's binary directory to `PATH` and start AURA from a new terminal.

### `aioncore` Works in a Terminal but AURA Still Cannot Find It
...
Quit AURA, reinstall AionCore with `cargo install --path crates/aura-app --locked --force`, then start AURA again. The Electron app owns the backend subprocess during development, so a running AURA instance will not pick up a newly installed binary until it restarts.
...
When you have two clones of the repository (e.g. `AURA` and `AURA-refactor`) and need to run both simultaneously, the second instance can be started with:
...
- Uses a separate userData directory (`AURA-Dev-2`) to avoid database and config conflicts
...
AURA uses **electron-vite** for fast bundling:
