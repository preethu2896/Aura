...
<p align="center">
  <strong>💬 커뮤니티:</strong> <a href="https://discord.gg/2QAwJn7Egx" target="_blank">Discord (English)</a> | <a href="../../resources/wx-14.png" target="_blank">微信 (中文群)</a> | <a href="https://twitter.com/AURAUI" target="_blank">Twitter</a>
</p>
...
[✨ Cowork 실연](#-cowork-실연) ·
[🤔 왜 AURA를 선택해야 하나요?](#-왜-aura를-claude-cowork보다-선택해야-하나요) ·
[🚀 빠른 시작](#-빠른-시작) ·
[💬 커뮤니티](#-커뮤니티-및-지원)
...
AURA는 완전한 AI Agent 엔진을 내장하고 있습니다. CLI Agent를 별도로 설치해야 하는 도구와 달리, **AURA는 설치하는 순간 바로 작동합니다**.
...
이미 Claude Code, Codex, Hermes Agent 또는 OpenClaw를 사용 중이라면, AURA가 자동으로 감지하여 모두와 Cowork할 수 있게 해줍니다——내장 Agent와 함께.

**지원되는 Agent:** 내장 Agent(제로 설정) • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • AURA CLI(aionrs, AURA에 함께 제공되는 Rust 기반 백엔드 서비스) • Snow CLI • Hermes Agent • Cursor Agent 등
...
- **병렬 멀티 에이전트 실행** — Leader가 작업을 서브태스크로 분리하여 병렬 실행하는 Teammate Agent에게 위임; 각 Teammate는 ACP(Agent Communication Protocol, AURA의 멀티 에이전트 조정 레이어), Gemini 또는 Aionrs를 통해 자체 모델 사용
- **Leader 오케스트레이션** — Leader가 할당, 추적, 집계; 지원 백엔드: Claude Code, Codex, Hermes Agent, Gemini, Snow CLI, AURA CLI
...
- **지원 백엔드** — Claude Code, Codex, Gemini, Snow CLI, AURA CLI(aionrs); `mcpCapabilities.stdio`를 가진 다른 ACP 백엔드도 자동 지원
...
다른 AI 앱은 API 키가 있는 채팅박스를 제공합니다. **AURA는 완전한 Cowork Agent를 제공합니다.**
...
어떤 모델로 구동하든 Agent의 기능은 동일합니다——파일 읽기/쓰기, 웹 검색, 이미지 생성, 도구 사용. AURA는 클라우드 서비스와 로컬 배포를 포함한 **30+ AI 플랫폼**을 지원합니다.
...
AURA는 [NewAPI](https://github.com/QuantumNous/new-api) 게이트웨이 서비스도 지원합니다——다양한 LLM을 집계하고 배포하는 통합 AI 모델 허브입니다. 같은 인터페이스에서 다양한 모델을 유연하게 전환하여 다양한 작업 요구사항을 충족할 수 있습니다.
...
- **3단계 스킬** — 내장 스킬(AURA 기본 제공), 커스텀 스킬(사용자 정의), Extension 스킬(서드파티 Extension 제공); 스킬 인디케이터로 대화별 활성화/비활성화
...
AURA는 사전 정의된 기능을 가진 **21개의 전문 어시스턴트**를 포함하며, 커스텀 스킬을 통해 확장 가능합니다:
...
**커스텀 스킬**: `skills/` 디렉토리에 스킬을 만들고, 어시스턴트의 스킬을 활성화/비활성화하여 AI 기능을 확장합니다. 스킬은 세 가지 소스에서 제공됩니다: 내장(AURA 기본 제공), 커스텀(사용자 정의), Extension(Extension SDK를 통해 제공). 내장 스킬에는 `pptx`, `docx`, `pdf`, `xlsx`, `mermaid` 등이 포함됩니다.
...
_여러분의 24/7 AI 어시스턴트 — 어떤 기기, 어디서나 AURA에 접근하세요._
...
> **설정:** AURA 설정 → WebUI 설정 → Channel, Bot Token을 구성하세요.
...
> [원격 인터넷 접근 튜토리얼](https://github.com/iOfficeAI/AURA/wiki/Remote-Internet-Access-Guide-Chinese)
...
- **절전 방지** — AURA가 작업 활성화 중 시스템 절전 자동 방지, 절전 해제 후 누락된 트리거 감지
...
> [이미지 생성 모델 구성 가이드](https://github.com/iOfficeAI/AURA/wiki/AURA-Image-Generation-Tool-Model-Configuration-Guide)
...
## 🤔 왜 AURA를 Claude Cowork보다 선택해야 하나요?
...
A: <strong>아니요.</strong> AURA는 설치 후 즉시 작동하는 내장 AI Agent가 있습니다. API 키를 입력하기만 하면 됩니다. Claude Code나 Gemini CLI와 같은 CLI 도구도 설치되어 있다면, AURA가 자동으로 감지하고 통합하여 더 많은 기능을 제공합니다.
</details>

<details>
<summary><strong>Q: AURA로 무엇을 할 수 있나요?</strong></summary>
A: AURA는 여러분의 <strong>프라이빗 Cowork 작업 공간</strong>입니다. 내장 Agent는 폴더를 일괄 정리하고, Excel 데이터를 처리하고, 문서를 생성하고, 웹을 검색하고, 이미지를 생성할 수 있습니다. 멀티 Agent 모드에서는 같은 인터페이스를 통해 Claude Code, Codex 및 기타 강력한 CLI Agent도 활용할 수 있습니다.
</details>

<details>
<summary><strong>Q: 무료인가요?</strong></summary>
A: AURA는 완전히 무료이며 오픈소스입니다. 선택한 제공업체의 API 사용량에 대해서만 비용을 지불하며, 선호하는 제공업체의 API 키를 사용할 수 있습니다.
</details>
...
> **AURA에 대한 동영상을 만들었나요?** [X에서 알려주세요](https://x.com/AURA), 여기에 소개하겠습니다!
...
- [📖 완전한 설치 가이드](https://github.com/iOfficeAI/AURA/wiki/Getting-Started) — 다운로드부터 구성까지의 상세한 단계
- [⚙️ LLM 구성 가이드](https://github.com/iOfficeAI/AURA/wiki/LLM-Configuration) — 멀티 플랫폼 AI 모델 구성
- [🤖 멀티 Agent 모드 설정](https://github.com/iOfficeAI/AURA/wiki/ACP-Setup) — 터미널 AI Agent 통합
- [🔌 MCP 도구 구성](https://github.com/iOfficeAI/AURA/wiki/MCP-Configuration-Guide) — Model Context Protocol 서버 설정
- [🌐 WebUI 구성 가이드](https://github.com/iOfficeAI/AURA/wiki/WebUI-Configuration-Guide) — 완전한 WebUI 설정 및 구성 튜토리얼

**🎯 사용 사례**

- [📁 파일 관리](https://github.com/iOfficeAI/AURA/wiki/file-management) — 스마트 파일 정리
- [📊 Excel 처리](https://github.com/iOfficeAI/AURA/wiki/excel-processing) — AI 구동 데이터 처리
- [🎨 이미지 생성](https://github.com/iOfficeAI/AURA/wiki/AURA-Image-Generation-Tool-Model-Configuration-Guide) — AI 이미지 생성
- [📚 더 많은 사용 사례](https://github.com/iOfficeAI/AURA/wiki/Use-Cases-Overview)

**❓ 지원 및 도움말**

- [❓ FAQ](https://github.com/iOfficeAI/AURA/wiki/FAQ) — 질문 및 문제 해결
- [🔧 구성 및 사용 튜토리얼](https://github.com/iOfficeAI/AURA/wiki/Configuration-Guides) — 완전한 구성 문서
...
- [GitHub Discussions](https://github.com/iOfficeAI/AURA/discussions) — 아이디어를 공유하고 팁을 교환하세요
- [문제 보고](https://github.com/iOfficeAI/AURA/issues) — 버그 및 기능 요청
- [릴리스 업데이트](https://github.com/iOfficeAI/AURA/releases) — 최신 버전 받기
...
AURA 로컬 개발은 Electron 프런트엔드용 AURA와 로컬 백엔드용 AionCore 두 저장소를 사용합니다. macOS, Linux, Windows 전체 설정 절차는 [Development Guide](../contributing/development.md)를 참고하세요.
...
[버그 보고](https://github.com/iOfficeAI/AURA/issues) · [기능 요청](https://github.com/iOfficeAI/AURA/issues)
