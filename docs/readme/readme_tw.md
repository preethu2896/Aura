...
[✨ Cowork 演示](#-cowork-演示) ·
[🤔 為什麼選擇 AURA？](#-為什麼選擇-aura-而非-claude-cowork) ·
[🚀 快速開始](#-快速開始) ·
[💬 社群](#-社群與支援)
...
**AURA 不只是個聊天工具。** 它是一個真正的 Cowork 平台，AI Agent 就像您的得力助手，在電腦上幫您處理各種任務——讀檔案、寫程式碼、查資料、自動化工作流。Agent 的一舉一動都在您的掌控之中，透明可見。

|                       | 傳統 AI 聊天客戶端 | **AURA (Cowork)**                                                                                      |
| :-------------------- | :----------------- | :----------------------------------------------------------------------------------------------------- |
...
AURA 自帶完整的 AI Agent 引擎。不像其他工具需要您手動安裝各種 CLI 工具，**AURA 裝好就能用，開箱即用**。
...
想把文件/表格直接交給 Agent？AURA 內建 **[OfficeCLI](https://github.com/iOfficeAI/OfficeCLI)**，讓 PPT（Morph 轉場）、Word（`.docx`）與 Excel（`.xlsx/.xlsm/.csv`）從需求到可交付結果更高效、更穩定。
...
如果您已經在用 Claude Code、Codex 或 Qwen Code，AURA 會自動發現它們，讓您同時和這些 Agent 一起 Cowork——當然，還有內建 Agent。

**支援的 Agent：** 內建 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • AURA CLI（aionrs，隨附於 AURA 的 Rust 後端服務） • Snow CLI • Hermes Agent • Cursor Agent 等
...
- **多 Agent 並行執行** — Leader 將任務分解為子任務並委派給並行運行的 Teammate Agent；每個 Teammate 透過 ACP（Agent Communication Protocol，AURA 的多 Agent 協調層）、Gemini 或 Aionrs 使用獨立模型
- **Leader 統籌編排** — Leader 分配、追蹤並彙總結果；支援的後端包括 Claude Code、Codex、Hermes Agent、Gemini、Snow CLI 和 AURA CLI
...
- **支援的後端** — Claude Code、Codex、Gemini、Snow CLI、AURA CLI（aionrs）；其他具備 `mcpCapabilities.stdio` 的 ACP 後端自動支援
...
其他 AI 應用可能只給您個聊天視窗，**但 AURA 給您的是完整的 Cowork Agent**。
...
| AWS Bedrock 憑證           | 透過 AURA CLI（aionrs）的 Bedrock 驅動 Agent |
| Ollama / LM Studio（本地） | 本地模型 Cowork Agent                        |
| NewAPI 閘道                | 統一存取 20+ 模型                            |

不管用哪個模型，Agent 的能力都一樣強大——檔案讀寫、網路搜尋、圖像生成、工具調用，一個不少。AURA 支援 **30+ 個 AI 平台**，雲端本地都能用。
...
AURA 還支援 [NewAPI](https://github.com/QuantumNous/new-api) 閘道服務 — 一個統一的 AI 模型中心，聚合和分發各種大語言模型。在同一個介面中靈活切換不同模型，滿足各種任務需求。
...
- **三層技能體系** — 內建技能（隨 AURA 附帶）、自訂技能（您自己的）以及擴展技能（第三方擴展貢獻）；透過技能指示器按對話啟用/停用
...
AURA 內建 **21 個專業助手**，每個都有獨特能力，還能透過自訂技能繼續擴展：
...
**自訂技能**：在 `skills/` 目錄下建立您的專屬技能，隨時為助手開啟或關閉，讓 AI 能力無限擴展。技能來源分三層：內建（隨 AURA 附帶）、自訂（您自己的）以及擴展（透過擴展 SDK 貢獻）。內建技能有 `pptx`、`docx`、`pdf`、`xlsx`、`mermaid` 等。
...
> **設定：** AURA 設定 → WebUI 設定 → Channel，配置 Bot Token。
...
> [遠端網際網路存取教學](https://github.com/iOfficeAI/AURA/wiki/Remote-Internet-Access-Guide-Chinese)
...
- **防休眠** — AURA 會自動阻止系統休眠，任務啟用期間偵測喚醒後的漏觸發
...
> [圖像生成模型配置指南](https://github.com/iOfficeAI/AURA/wiki/AURA-Image-Generation-Tool-Model-Configuration-Guide)
...
AURA 是一個**免費開源的 Multi-AI Agent 桌面應用**。相比只能在 macOS 上用、還只能綁定 Claude 的 Claude Cowork，AURA 支援全模型、跨平台，是它的全面升級版。

| 維度     | Claude Cowork | AURA                                                   |
| :------- | :------------ | :----------------------------------------------------- |
...
答：<strong>完全不需要。</strong> AURA 自帶 AI Agent，裝好就能用。輸入任意 API 金鑰即可開始。如果您已經裝了 Claude Code 或 Gemini CLI 這些 CLI 工具，AURA 會自動發現並整合它們，功能更強大。
</details>

<details>
<summary><strong>問：我可以用 AURA 做什麼？</strong></summary>
答：AURA 就是您的<strong>私有 Cowork 工作空間</strong>。內建 Agent 可以幫您批次整理資料夾、處理 Excel 資料、產生文件、搜尋網路、產生圖像。透過多 Agent 模式，您還能在同一介面同時使用 Claude Code、Codex 和其他強大的 CLI Agent。
</details>

<details>
<summary><strong>問：它是免費的嗎？</strong></summary>
答：AURA 完全免費且開源。您只需為所選服務商的 API 用量付費，可以使用任何您喜歡的服務商的 API 金鑰。
</details>
...
> **製作了關於 AURA 的影片？** [在 X 上告訴我們](https://x.com/AURA)，我們會在這裡展示！
...
- [📖 完整安裝指南](https://github.com/iOfficeAI/AURA/wiki/Getting-Started) — 從下載到配置，一步步教您
- [⚙️ LLM 配置指南](https://github.com/iOfficeAI/AURA/wiki/LLM-Configuration) — 多平台 AI 模型怎麼配置
- [🤖 多 Agent 模式設定](https://github.com/iOfficeAI/AURA/wiki/ACP-Setup) — 把終端 AI Agent 整合進來
- [🔌 MCP 工具配置](https://github.com/iOfficeAI/AURA/wiki/MCP-Configuration-Guide) — 模型上下文協定伺服器設定
- [🌐 WebUI 配置指南](https://github.com/iOfficeAI/AURA/wiki/WebUI-Configuration-Guide) — WebUI 完整設定教學

**🎯 使用場景**

- [📁 檔案管理](https://github.com/iOfficeAI/AURA/wiki/file-management) — 讓 AI 幫您整理檔案
- [📊 Excel 處理](https://github.com/iOfficeAI/AURA/wiki/excel-processing) — AI 驅動的資料處理
- [🎨 圖像生成](https://github.com/iOfficeAI/AURA/wiki/AURA-Image-Generation-Tool-Model-Configuration-Guide) — AI 圖像生成
- [📚 更多使用場景](https://github.com/iOfficeAI/AURA/wiki/Use-Cases-Overview)

**❓ 支援與幫助**

- [❓ FAQ](https://github.com/iOfficeAI/AURA/wiki/FAQ) — 常見問題和解決方案
- [🔧 配置與使用教學](https://github.com/iOfficeAI/AURA/wiki/Configuration-Guides) — 完整配置文件
...
- [GitHub Discussions](https://github.com/iOfficeAI/AURA/discussions) — 分享想法，交流使用技巧
- [報告問題](https://github.com/iOfficeAI/AURA/issues) — 遇到 bug 或有新功能想法？告訴我們
- [發布更新](https://github.com/iOfficeAI/AURA/releases) — 取得最新版本
...
AURA 本機開發涉及兩個倉庫：AURA 負責 Electron 前端，AionCore 負責本機後端。完整的 macOS、Linux 和 Windows 設定流程請見 [開發指南](../contributing/development.md)。
...
<td>
  <a href="https://packycode.com" target="_blank">PackyCode</a> 是一家可靠高效的 API 中繼服務提供商，為 Claude Code、Codex、Gemini 等平台提供中繼服務。感謝 PackyCode 為支持 AURA 用戶低成本使用，為我們的用戶提供專屬 <a href="https://www.packyapi.com/register?aff=aura" target="_blank">9折優惠</a>，點擊此連結並在付款時使用優惠碼 <code>aura</code> 可立減 10%。
</td>
</tr>
</table>
...
[報告 Bug](https://github.com/iOfficeAI/AURA/issues) · [請求功能](https://github.com/iOfficeAI/AURA/issues)
