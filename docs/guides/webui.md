# AURA WebUI Mode - Startup Guide

AURA supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts AURA with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use AURA from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\AURA\AURA.exe" --webui

# Or if AURA is in your PATH
AURA.exe --webui
```
...
```bash
# Using full path
/Applications/AURA.app/Contents/MacOS/AURA --webui

# Or using open command
open -a AURA --args --webui
```

### Method 2: Create Shell Script

Create `start-aura-webui.sh`:
...
Create `~/.local/share/applications/aura-webui.desktop`:

```ini
[Desktop Entry]
Name=AURA WebUI
Comment=Start AURA in WebUI mode
Exec=/opt/AURA/aura --webui
Icon=aura
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/aura-webui.desktop
```
...
Create `~/bin/start-aura-webui.sh`:
...
```bash
chmod +x ~/bin/start-aura-webui.sh
```
...
Create `/etc/systemd/system/aura-webui.service`:

```ini
[Unit]
Description=AURA WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/AURA/aura --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable aura-webui.service
sudo systemctl start aura-webui.service

# Check status
sudo systemctl status aura-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run AURA in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making AURA accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running AURA WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/iOfficeAI/AURA/issues/217)
...
#### 4. Download and Install AURA

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/iOfficeAI/AURA/releases
wget https://github.com/iOfficeAI/AURA/releases/download/vVERSION/AURA_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/iOfficeAI/AURA/releases/download/vVERSION/AURA_VERSION_arm64.deb

# Install the package
apt install -y ./AURA_*.deb
...
```bash
# Start AURA in WebUI mode with no-sandbox flag
AURA --no-sandbox --webui
```
...
To access AURA from other devices on your local network:

```bash
# Start with --remote flag
AURA --no-sandbox --webui --remote
...
AURA --no-sandbox --webui --port 8080
```
...
chmod +x /opt/AURA/aura
```

#### Out of Memory

AURA requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if AURA is running: look for "Server started" message
...
4. **Keep device charged** - Running AURA consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **AURA Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch AURA quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-aura.sh << 'EOF'
#!/bin/bash
echo "Starting AURA WebUI..."
AURA --no-sandbox --webui --remote
EOF
...
```bash
proot-distro login ubuntu -- bash -c "AURA --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/iOfficeAI/AURA/issues/217)
...
```cmd
AURA.exe --webui --remote
```

### macOS

```bash
/Applications/AURA.app/Contents/MacOS/AURA --webui --remote
```
...
netsh advfirewall firewall add rule name="AURA WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add AURA
...
```powershell
& "C:\Program Files\AURA\AURA.exe" --webui 2>&1 | Tee-Object -FilePath aura.log
```

**macOS/Linux:**

```bash
/path/to/aura --webui 2>&1 | tee aura.log
```
...
| Platform | Location                                               |
| -------- | ------------------------------------------------------ |
| Windows  | `%APPDATA%/AURA/webui.config.json`                     |
| macOS    | `~/Library/Application Support/AURA/webui.config.json` |
| Linux    | `~/.config/AURA/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\AURA\AURA.exe" --resetpass

# Or for a specific user
"C:\Program Files\AURA\AURA.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/AURA.app/Contents/MacOS/AURA --resetpass

# Or for a specific user
/Applications/AURA.app/Contents/MacOS/AURA --resetpass username
```
...
# Or for a specific user
aura --resetpass username

# Or using full path
/opt/AURA/aura --resetpass
```
...
- [GitHub Issues](https://github.com/iOfficeAI/AURA/issues)
...
2. Search [existing issues](https://github.com/iOfficeAI/AURA/issues)
3. Create a [new issue](https://github.com/iOfficeAI/AURA/issues/new) with:
   - Your OS and version
   - AURA version
   - Steps to reproduce
...
**Happy using AURA in WebUI mode!** 🚀
