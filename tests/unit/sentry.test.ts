...
describe('listLogFilesSync', () => {
  it('finds log files under dated year/month/day directories', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'aura-sentry-logs-'));
    try {
...
describe('captureBackendStartupFailure', () => {
  ...
  it('sets flattened incomplete-installation tags for update-related missing directory resources', async () => {
    ...
    try {
      ...
      error.details = {
        ...
        resourcesPath: 'C:\\Users\\alice\\AppData\\Local\\Programs\\AURA\\resources',
      ...
      expect(scopeSetTag).toHaveBeenCalledWith(
        'aura.backend_startup.incomplete_installation_kind',
        'missing_directory_resources'
      );
      expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.missing_bundled_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.missing_runtime_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.missing_binary', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.missing_hub_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.last_update_status', 'quit-and-install');
      expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.seconds_since_quit_and_install', '46');
      expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.install_path_kind', 'user_local_programs');
      expect(scopeSetContext).toHaveBeenCalledWith(
  ...
  it('sets bucketed health polling tags for backend startup timeouts', async () => {
    ...
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.health_polling_delayed', 'true');
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.health_attempts_bucket', '1');
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.health_attempt_deficit_bucket', '76-150');
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.health_timeout_overrun_bucket', 'over_60s');
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.health_max_attempt_gap_bucket', '0ms');
  });
  ...
  it('sets backend data migration reason and boundary tags', async () => {
    ...
    error.details = {
      ...
      stderrTail:
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.migration databasePath=/db/aura-backend.db: failed to initialize application data',
    };

    await captureBackendStartupFailure(error);

    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.reason', 'backend_data_migration_failed');
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.boundary_code', 'BOOTSTRAP_DATA_INIT_FAILED');
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.boundary_stage', 'database.migration');
  });
  ...
  it('sets local data repair reason and issue-kind tags', async () => {
    ...
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.reason', 'backend_local_data_repair_failed');
    expect(scopeSetTag).toHaveBeenCalledWith(
      'aura.backend_startup.local_data_issue_kind',
      'agent_metadata_invalid_utf8'
    );
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.boundary_code', 'BOOTSTRAP_SERVICE_INIT_FAILED');
    expect(scopeSetTag).toHaveBeenCalledWith('aura.backend_startup.boundary_stage', 'services.init');
  });
});
...
describe('initSentry beforeSend', () => {
  ...
  it('keeps user feedback reports even when diagnostics contain backend secondary text', () => {
    ...
    const event = {
      tags: {
        type: 'user-feedback',
        'aura.installation_integrity.user_report': 'true',
      },
      extra: {
