...
const makeGitHubReleaseResponse = () => [
  {
    ...
    name: 'v1.9.22',
    body: 'release notes',
    html_url: 'https://github.com/iOfficeAI/AURA/releases/tag/v1.9.22',
    ...
    assets: [
      ...
      {
        name: 'AURA-1.9.22-win-x64.exe',
        browser_download_url: 'https://github.com/iOfficeAI/AURA/releases/download/v1.9.22/AURA-1.9.22-win-x64.exe',
        size: 456,
...
describe('updateBridge CDN URL rewriting', () => {
  ...
  it('rewrites asset.url to the CDN path and keeps GitHub URL in fallbackUrl', async () => {
    ...
    try {
      ...
      const result = await handler({ repo: 'iOfficeAI/AURA' });
      ...
      const macAsset = assets.find((a: { name: string }) => a.name === 'AURA-1.9.22-mac-arm64.dmg');
      expect(macAsset).toBeDefined();
      expect(macAsset?.url).toBe('https://static.aionui.com/releases/1.9.22/AURA-1.9.22-mac-arm64.dmg');
      expect(macAsset?.fallbackUrl).toBe(
        'https://github.com/iOfficeAI/AURA/releases/download/v1.9.22/AURA-1.9.22-mac-arm64.dmg'
      );

      const linuxAsset = assets.find((a: { name: string }) => a.name === 'AURA-1.9.22-linux-amd64.deb');
      expect(linuxAsset?.url).toBe('https://static.aionui.com/releases/1.9.22/AURA-1.9.22-linux-amd64.deb');
    } finally {
      vi.unstubAllGlobals();
    }
  });
  ...
  it('accepts static.aionui.com URLs for download', async () => {
    ...
    try {
      ...
      const result = await handler({
        ...
        url: 'https://static.aionui.com/releases/1.9.22/AURA-1.9.22-mac-arm64.dmg',
        file_name: 'AURA-1.9.22-mac-arm64.dmg',
      });
