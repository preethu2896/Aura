...
describe('AutoUpdaterService', () => {
  ...
  it('does not use the stable CDN updater when prerelease manual mode is enabled', async () => {
    autoUpdaterMock.checkForUpdates.mockResolvedValue({
      ...
      updateInfo: {
        version: '2.1.14',
        files: [{ url: 'AURA-2.1.14-mac-arm64.dmg', sha512: 'sha512-value' }],
        path: 'AURA-2.1.14-mac-arm64.dmg',
        sha512: 'sha512-value',
  ...
  it('restores a completed cached auto-update when the downloaded package validates', async () => {
    const updateInfo = {
      version: '2.1.14',
      files: [{ url: 'AURA-2.1.14-mac.zip', sha512: 'sha512-value' }],
      path: 'AURA-2.1.14-mac.zip',
      sha512: 'sha512-value',
      releaseDate: '2026-06-08T00:00:00.000Z',
    };
    const fileInfo = {
      url: new URL('https://static.aionui.com/releases/2.1.14/AURA-2.1.14-mac.zip'),
      info: { url: 'AURA-2.1.14-mac.zip', sha512: 'sha512-value' },
    };
    const cachedUpdatePath = path.join('/cache/pending', 'AURA-2.1.14-mac.zip');
