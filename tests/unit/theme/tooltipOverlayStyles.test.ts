...
describe('arco tooltip and popover overlay styles', () => {
  it('defines shared light and dark overlay tokens for tooltip-like surfaces', () => {
    ...
    expect(css).toContain('--aura-overlay-bg: #ffffff;');
    expect(css).toContain('--aura-overlay-text: #1d2129;');
    expect(css).toContain("body[arco-theme='dark'] {");
    expect(css).toContain('--aura-overlay-bg: #0e0e0e;');
    expect(css).toContain('--aura-overlay-text: #f2f3f5;');
  });
  ...
  it('applies the shared overlay tokens to tooltip, popover, and popconfirm surfaces', () => {
    ...
    expect(css).toContain('background: var(--aura-overlay-bg) !important;');
    expect(css).toContain('color: var(--aura-overlay-text) !important;');
    expect(css).toContain('border: 1px solid var(--aura-overlay-border) !important;');
