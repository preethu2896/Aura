...
describe('release packaging configuration', () => {
  ...
  it('uploads mac zip artifacts without a stale Windows zip glob', () => {
    ...
    expect(workflow).toContain('out/AURA-*-mac-*.zip');
    expect(workflow).not.toContain('out/AURA-*-win32-*.zip');
  });
  ...
  it.skipIf(process.platform === 'win32')('fails release asset preparation when a mac zip is missing', () => {
    const tempDir = mkdtempSync(resolve(tmpdir(), 'aura-release-assets-'));
    ...
    try {
      ...
      rmSync(resolve(artifactsDir, 'macos-build-arm64', 'AURA-1.0.0-mac-arm64.zip'), { force: true });
