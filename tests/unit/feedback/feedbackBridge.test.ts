...
vi.mock('electron', () => ({
  ...
  app: {
    getPath: vi.fn(() => '/tmp/aura-test-logs-nonexistent'),
...
describe('feedback logs', () => {
  it('collects top-level frontend logs and nested backend logs through the IPC handler', async () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'aura-feedback-bridge-'));
    try {
  ...
  it('collects the same recent three log days used by user feedback reports', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'aura-feedback-logs-'));
    try {
  ...
  it('collects recent logs from dated year/month/day directories', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'aura-feedback-dated-logs-'));
    try {
