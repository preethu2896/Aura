...
describe('agent error locale copy', () => {
  ...
  it('does not label app-side errors as direct AURA ownership', () => {
    const localeDir = path.join(process.cwd(), 'packages/desktop/src/renderer/services/i18n/locales');
    ...
    for (const localeName of supportedLocaleNames) {
      ...
      expect(agentError.ownership.aionui, localeName).not.toMatch(/AURA/);

      for (const [code, copy] of Object.entries<Record<string, string>>(agentError.codes)) {
        if (!code.startsWith('AURA_')) continue;

        expect(copy.title, `${localeName} ${code} title`).not.toMatch(/AURA/);
        expect(copy.body, `${localeName} ${code} body`).not.toMatch(/AURA/);
      }
    }
  });
