...
vi.mock('electron', () => ({
  app: {
    ...
    getPath: vi.fn(() => '/tmp/aura-update-dedupe-test'),
    exit: vi.fn(),
    isPackaged: true,
  },
}));
...
describe('updateBridge manual download dedupe', () => {
  ...
  it('reuses the active manual download for the same URL, fallback URL, and file name', async () => {
    ...
    const request = {
      url: 'https://static.aionui.com/releases/2.2.0/AURA-2.2.0-mac-arm64.dmg',
      fallbackUrl: 'https://github.com/iOfficeAI/AURA/releases/download/v2.2.0/AURA-2.2.0-mac-arm64.dmg',
      file_name: 'AURA-2.2.0-mac-arm64.dmg',
    };
  ...
  it('creates a new manual download after the prior matching task reaches a terminal state', async () => {
    fs.mkdirSync('/tmp/aura-update-dedupe-test', { recursive: true });
    vi.stubGlobal(
      'fetch',
