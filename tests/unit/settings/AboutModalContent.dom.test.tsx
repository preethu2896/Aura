...
describe('AboutModalContent update ready state', () => {
  ...
  it('replaces check update with ready-to-install when an update package is ready', async () => {
    ...
    await act(async () => {
      window.dispatchEvent(
        new CustomEvent('aura-update-ready-state-changed', {
          detail: {
            ready: true,
            version: '2.1.14',
          },
  ...
  it('reveals the notification card only when an update is available, with no toast', async () => {
    ...
    window.addEventListener('aura-update-available', availableListener);
    ...
    window.removeEventListener('aura-update-available', availableListener);
  });
