...
vi.mock('@/renderer/components/base/AURAModal', () => ({
...
describe('UpdateModal manual install fallback', () => {
  beforeEach(() => {
    ...
    mocks.updateCheckMock.mockResolvedValue({
      ...
      data: {
        ...
        latest: {
          ...
          name: 'v2.1.14',
          body: 'notes',
          htmlUrl: 'https://github.com/iOfficeAI/AURA/releases/tag/v2.1.14',
          prerelease: false,
          draft: false,
          assets: [],
          recommendedAsset: {
            name: 'AURA-2.1.14-mac-arm64.dmg',
            url: 'https://static.aionui.com/releases/2.1.14/AURA-2.1.14-mac-arm64.dmg',
            fallbackUrl: 'https://github.com/iOfficeAI/AURA/releases/download/v2.1.14/AURA-2.1.14-mac-arm64.dmg',
            size: 123,
          },
        },
      },
    ...
    mocks.updateDownloadMock.mockImplementation(async (request: UpdateDownloadRequest) => {
      ...
      mocks.manualProgressHandler?.({
        ...
        totalBytes: 123,
        percent: 100,
        file_path: '/tmp/AURA-2.1.14-mac-arm64.dmg',
      });
      return {
        success: true,
        data: {
  ...
  it('keeps fast manual download completion matched to the caller-provided download id', async () => {
    ...
    act(() => {
      window.dispatchEvent(new Event('aura-open-update-modal'));
    });
    ...
    expect(mocks.updateDownloadMock).toHaveBeenCalledWith({
      ...
      url: 'https://static.aionui.com/releases/2.1.14/AURA-2.1.14-mac-arm64.dmg',
      fallbackUrl: 'https://github.com/iOfficeAI/AURA/releases/download/v2.1.14/AURA-2.1.14-mac-arm64.dmg',
      file_name: 'AURA-2.1.14-mac-arm64.dmg',
    });
