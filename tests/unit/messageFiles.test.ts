...
describe('buildDisplayMessage', () => {
  const workspace = '/tmp/aura/workspace-1';
  ...
  it('strips AURA timestamp separators from filenames while keeping prefix', () => {
    const files = [`${workspace}/uploads/photo_aura_1234567890123.jpg`];
    const result = buildDisplayMessage('hello', files, workspace);
    expect(result).toContain(`${workspace}/uploads/photo.jpg`);
  });
});
