...
describe('useGuidSend', () => {
  ...
  it('forwards local skill overrides for generated AURA CLI assistants through assistant conversation overrides', async () => {
    const deps = createDeps();
  ...
  it('does not write legacy preset_assistant_id for generated AURA CLI assistant conversations', async () => {
    const deps = createDeps();
    deps.selectedAssistantId = 'bare:aionrs';
    deps.selectedAssistantBackend = 'aionrs';
    deps.current_model = { provider_id: 'openai', model: 'gemini-2.5-pro', use_model: 'gemini-2.5-pro' } as never;
