...
vi.mock('@/renderer/components/base/AURAModal', () => ({ default: () => null }));
...
describe('LocalAgents', () => {
  ...
  it('reads the managed-agents view and renders detected + custom sections', () => {
    ...
    expect(screen.getByText('AURA CLI')).toBeTruthy();
  ...
  it('shows a lightweight refresh hint while the management view is revalidating', () => {
    ...
    expect(screen.getByText('AURA CLI')).toBeInTheDocument();
  });
  ...
  it('renders the setup-guide action for official agents diagnostics', () => {
    ...
    expect(openExternalUrl).toHaveBeenCalledWith('https://github.com/iOfficeAI/AURA/wiki/ACP-Setup');
  });
