...
4. **后端持久化**
   - backend 独占 `aura.db`，Electron 不再直接访问 SQLite
...
- 解析顺序: 环境变量 `AION_CLI_PATH` → `~/.aionui/bin/aura-<platform>-<arch>` → 系统 PATH 中的 `aura` 命令
...
| `aioncore aura.db`                                                          | —      | conversations + messages 由 backend 独占持久化                      |
