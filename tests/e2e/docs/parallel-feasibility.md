...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aura-e2e-state-'));
...
**Critical bottleneck**: `workers: 1` enforced + shared `aura.db` → parallel execution impossible without refactoring.
...
2. Modify `getDevAppName()` to return worker-specific name:
   ...
   return `AURA-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/AURA-E2E-Worker-0/config/aura.db`
   - `~/Library/Application Support/AURA-E2E-Worker-1/config/aura.db`
