...
async function closeFeedbackModal(page: Page) {
  ...
  // button class — scoped to avoid matching the Agent editor's AURAModal
...
/** Close any open AURAModal (e.g. the Agent editor) so the next test starts clean. */
...
async function openCustomAgentEditor(page: Page, command: string) {
  // Defensive: close any AURAModal left over from a prior test so the
...
test('[5] Agent fail_cli alert surfaces feedback pill (module=agent-detection)', async ({ page }) => {
  await openCustomAgentEditor(page, 'aura-e2e-missing-binary-xyz');
