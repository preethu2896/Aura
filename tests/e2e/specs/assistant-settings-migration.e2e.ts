 ...
 * 1. seed a legacy `aura.db` using the pre-unification assistants schema
...
function schemaPath(): string {
  return path.join(process.cwd(), '../aionCore/crates/aura-db/migrations/001_initial_schema.sql');
}

function querySqliteValue(dataDir: string, sql: string): string {
  const dbPath = path.join(dataDir, 'aura-backend.db');
  return execFileSync('sqlite3', ['-readonly', dbPath, sql], { encoding: 'utf8' }).trim();
}

function seedLegacyDatabase(dataDir: string): void {
  const legacyDbPath = path.join(dataDir, 'aura.db');
