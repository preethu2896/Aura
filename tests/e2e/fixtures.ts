 ...
 *      (e.g. out/linux-unpacked/aura, out/mac-arm64/AURA.app, out/win-unpacked/AURA.exe).
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/AURA.exe  or  out/win-x64-unpacked/AURA.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'AURA.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/AURA.app/Contents/MacOS/AURA  or  out/mac/AURA.app/...
