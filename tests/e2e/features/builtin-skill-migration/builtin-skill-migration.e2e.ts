 ...
 */
const REMOVED_AUTO_INJECT_NAME = 'aura-skills';
const REMOVED_AUTO_INJECT_DIR_NAME = 'aura-skills';
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  test('S7: builtin skills in /api/skills expose an absolute, readable location for export', async ({ page }) => {
    ...
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aura-e2e-s7-export-'));
    try {
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    test.beforeEach(() => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aura-e2e-builtin-skill-'));
    });
