 ...
 * (`/Users/zhoukai/Downloads/aura-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `aura-config.txt` exactly as ConfigStorage would.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/aura-config.txt';
...
/**
 * Decode an `aura-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'aura-migrate-fixture-'));
