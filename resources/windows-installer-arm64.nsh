...
!define AIONUI_APP_EXECUTABLE_FILENAME "AURA.exe"
!define AIONUI_PROCESS_CHECK_LOG "aura-installer-process-check.log"

!ifndef BUILD_UNINSTALLER
  Var /GLOBAL AURAUninstallHadErrors
  Var /GLOBAL AURAUninstallLogResult
  Var /GLOBAL AURAVerifyResourceResult
!endif
...
!macro AIONUI_LOG_UNINSTALLER_REPAIR _PHASE
  ...
  Pop $AURARepairLogResult
!macroend

!macro AIONUI_REPAIR_INSTALLED_UNINSTALLER
  Var /GLOBAL AURAInstalledUninstaller
  Var /GLOBAL AURABundledUninstaller
  Var /GLOBAL AURARepairLogResult

  !insertmacro AIONUI_LOG_UNINSTALLER_REPAIR "before"
  StrCpy $AURAInstalledUninstaller "$INSTDIR\${UNINSTALL_FILENAME}"

  ${If} ${FileExists} "$AURAInstalledUninstaller"
    InitPluginsDir
    StrCpy $AURABundledUninstaller "$PLUGINSDIR\AURA-fixed-uninstaller.exe"
    SetOverwrite on
    File "/oname=$PLUGINSDIR\AURA-fixed-uninstaller.exe" "${UNINSTALLER_OUT_FILE}"

    ClearErrors
    CopyFiles /SILENT "$AURABundledUninstaller" "$AURAInstalledUninstaller"
    ${If} ${Errors}
      !insertmacro AIONUI_LOG_UNINSTALLER_REPAIR "copy-failed"
      MessageBox MB_OK|MB_ICONEXCLAMATION "AURA cannot update because the existing uninstaller is locked.$\r$\n$\r$\nPlease close AURA completely and try again. If it still fails, restart Windows and run this installer again.$\r$\n$\r$\nIf the problem continues, uninstall the old AURA from Windows Settings, then run this installer again."
      SetErrorLevel 2
      Quit
    ${Else}
...
!macro customCheckAppRunning
  Var /GLOBAL AURACheckResult
  Var /GLOBAL AURACloseRetries
  Var /GLOBAL AURAStopResult

  !insertmacro AIONUI_FIND_APP_PROCESS $AURACheckResult
  ${If} $AURACheckResult == 0
    MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION "$(appRunning)" /SD IDOK IDOK aura_do_stop_process
    Quit

    aura_do_stop_process:
      DetailPrint "$(appClosing)"
      !insertmacro AIONUI_STOP_APP_PROCESSES
      StrCpy $AURACloseRetries 0

    aura_wait_for_close:
      Sleep 1000
      !insertmacro AIONUI_FIND_APP_PROCESS $AURACheckResult
      ${If} $AURACheckResult == 0
        IntOp $AURACloseRetries $AURACloseRetries + 1
        ${If} $AURACloseRetries > 10
          MessageBox MB_RETRYCANCEL|MB_ICONEXCLAMATION "$(appCannotBeClosed)" /SD IDCANCEL IDRETRY aura_wait_for_close
          Quit
        ${Else}
          !insertmacro AIONUI_STOP_APP_PROCESSES
          Goto aura_wait_for_close
        ${EndIf}
      ${EndIf}
  ${EndIf}
!macroend
...
!macro AIONUI_VERIFY_REQUIRED_FILE _PATH _LABEL
  ${IfNot} ${FileExists} "${_PATH}"
    ...
    MessageBox MB_OK|MB_ICONSTOP \
      "AURA installation is incomplete.$\n$\n\
      Missing required file: ${_LABEL}$\n\
      Path: ${_PATH}$\n$\n\
      Please reinstall AURA or download a newer installer." \
      /SD IDOK
    SetErrorLevel 3
    Quit
  ${Else}
...
!macro AIONUI_VERIFY_ARM64_APP_FILES
  ...
  !insertmacro AIONUI_VERIFY_REQUIRED_FILE "$INSTDIR\AURA.exe" "AURA.exe"
...
!macro AIONUI_VERIFY_BUNDLED_AIONCORE_RESOURCES _RUNTIME_KEY
  ...
  Pop $AURAVerifyResourceResult

  ${If} $AURAVerifyResourceResult != 0
...
!macro AIONUI_HANDLE_UNINSTALL_RESULT _ROOT_KEY
  ${If} ${Errors}
    StrCpy $AURAUninstallHadErrors "1"
  ${Else}
    StrCpy $AURAUninstallHadErrors "0"
  ${EndIf}

  !insertmacro AIONUI_LOG_UNINSTALL_RESULT "${_ROOT_KEY}" "$AURAUninstallHadErrors"

  ${If} $AURAUninstallHadErrors == "1"
...
!macro customRemoveFiles
  ...
  StrCpy $R1 ""
  Var /GLOBAL AURARemoveDirResult
...
Function .onVerifyInstDir
  ...
  ${IfNot} ${IsNativeARM64}
    ...
    MessageBox MB_OK|MB_ICONSTOP \
      ...
      This AURA installer is designed for ARM64 architecture.$\n$\n\
      Your system does not support ARM64. Please download the appropriate version for your architecture.$\n$\n\
      Download: https://github.com/iOfficeAI/AURA/releases"
    Quit
  ${EndIf}
FunctionEnd
