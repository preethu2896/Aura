...
- **运行环境**: GUI 模式（AURA 运行中）

### 1.2 功能描述

1. 与 WebUI 功能类似，用户可通过个人终端直接使用 AURA 功能
...
过程: 平台机器人接收消息 → 转发给 AURA Agent → LLM 处理
...
├─────────────────────────────────────────────────────────────┤
│ Step 4: 配置凭证                                            │
│   AURA 设置页面 → 粘贴 App ID、App Secret → 验证 → 保存   │
├─────────────────────────────────────────────────────────────┤
│ Step 5: 启动 Bot                                            │
...
与 Telegram 相同，采用本地批准模式。配对码通过 Lark 消息发送给用户，用户在 AURA 中批准。
