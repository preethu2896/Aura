#!/usr/bin/env bash
# ============================================================================
# AURA WebUI — One-Click Installation Script
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/iOfficeAI/AURA/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/aura-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/aura-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/iOfficeAI/AURA/releases/download}"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║     AURA WebUI Installer (No Electron)     ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/aura-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="aura-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/iOfficeAI/AURA/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is aura-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/aura-web" ]]; then
        mv "${extract_temp}/aura-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing aura-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/aura-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/aura-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/aura-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/aura-web"
    local target_path="${INSTALL_DIR}/aura-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by aura-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 AURA WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/aura-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start AURA WebUI"
        echo "    aura-web start"
        echo ""
        echo "    # Check version"
        echo "    aura-web version"
    else
        echo "    # Start AURA WebUI (using full path)"
        echo "    ${INSTALL_DIR}/aura-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/aura-web ~/.local/bin/aura-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/iOfficeAI/AURA"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/iOfficeAI/AURA/issues"
    echo ""
