 ...
 * parsing the electron-log file for [AURA:ready] / [AURA:init] /
 * [AURA:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  // Parsed from [AURA:init] marks
  initTotalMs: number;
  // Parsed from [AURA:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'AURA-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'AURA', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'AURA', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'AURA', 'logs', `${today}.log`));
  }
...
// Matches: [AURA:ready] <label> +<ms>ms
// Matches: [AURA:init]  <label> +<ms>ms
// Matches: [AURA:process] <label> +<ms>ms
const MARK_REGEX = /\[AURA:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[AURA] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[AURA] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[AURA] Showing main window')) marks.logs.showingMainWindow = true;
  }
