...
const tarballName = `aura-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'aura-web');
...
const executableName = platform === 'win32' ? 'aura-web.exe' : 'aura-web';
...
// `aura-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} aura-web`, {
