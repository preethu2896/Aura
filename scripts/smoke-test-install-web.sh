...
export INSTALL_DIR="/tmp/aura-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/aura-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/aura-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/aura-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/aura-web"
    exit 1
fi
...
VERSION_OUTPUT=$(aura-web version 2>&1 || echo "")
