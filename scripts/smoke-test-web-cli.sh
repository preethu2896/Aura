...
if [ ! -d "$TEMP_DIR/aura-web" ]; then
  echo "❌ Missing aura-web directory"
  exit 1
fi

cd "$TEMP_DIR/aura-web"

# New layout (bun compile standalone binary):
#   aura-web/
#   ├── aura-web           ← single compiled executable (no bin/, no dist/, no node_modules)
...
if [ ! -x "aura-web" ]; then
  echo "❌ aura-web is not executable"
  exit 1
fi
echo "✓ aura-web is executable"

# 4. Test version command
echo ""
echo "4. Testing version command..."
VERSION=$(./aura-web version)
if [ -z "$VERSION" ]; then
...
HTTP_PORT=25899
DATA_DIR="$(mktemp -d)/aura-web-data"
...
./aura-web start --port "$HTTP_PORT" --data-dir "$DATA_DIR" > /tmp/aura-web.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${HTTP_PORT}/" > /tmp/aura-web.html 2>/dev/null; then
    break
  fi
  sleep 1
done

if [ ! -s /tmp/aura-web.html ]; then
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP probe failed — no response body. Server log:"
  cat /tmp/aura-web.log
  exit 1
fi

# Look for the SPA shell signature — <html + <div id="root" or similar marker
if grep -q '<html' /tmp/aura-web.html && grep -qE '<(div id="root"|script)' /tmp/aura-web.html; then
  echo "✓ HTTP root returns SPA index ($(wc -c < /tmp/aura-web.html) bytes)"
else
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP root response does not look like SPA index:"
  head -20 /tmp/aura-web.html
  echo "---server log---"
  cat /tmp/aura-web.log
  exit 1
fi
...
if grep -q 'Backend binary not found' /tmp/aura-web.log; then
  ...
  for i in $(seq 1 20); do
    PASSWORD=$(grep -oE 'Generated initial admin password: [^ ]+' /tmp/aura-web.log | head -1 | sed 's/^Generated initial admin password: //')
    if [ -n "$PASSWORD" ]; then
      break
    fi
    sleep 1
