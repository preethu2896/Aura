...
/**
 * Simplified build script for AURA
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const auraRunning = isProcessRunningWindows('AURA.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (auraRunning || electronRunning) {
        console.log('⚠️  Detected running AURA/Electron process. Attempting to close...');
        killWindowsProcesses(['AURA.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running AURA/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'AURA.exe');
    ...
    console.log('⚠️  Windows local build failed after AURA.exe was produced.');
    if (firstError) {
