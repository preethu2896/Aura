...
const KILLABLE_NAMES = new Set(['electron', 'aura', 'aura.exe']);
...
function listLikelyConflictingProcesses() {
  try {
    if (isWindows()) {
      const output = run(
        "powershell -NoProfile -Command \"Get-Process | Where-Object { $_.ProcessName -in @('electron','AURA','node','bun') } | Select-Object ProcessName,Id | ConvertTo-Json -Compress\""
      );
      const parsed = output ? JSON.parse(output) : [];
      return Array.isArray(parsed) ? parsed : [parsed];
    }

    const output = run(`ps -A -o pid=,comm= | egrep "electron|AURA|node|bun" || true`);
    return output
      .split(/\r?\n/)
      .filter(Boolean)
      .map((line) => {
...
function cleanupByName() {
  ...
  for (const proc of processes) {
    ...
    if (!['electron', 'aura'].some((k) => rawName.includes(k))) continue;
    if (killPid(pid)) {
